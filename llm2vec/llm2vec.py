...
class LatentAttentionPooling(nn.Module):
    def __init__(self, hidden_size, num_latents=512, num_heads=8):
        super().__init__()
        ...
        # We use batch_first=True so that input shape is (batch, seq_length, hidden_size)
        self.multihead_attn = nn.MultiheadAttention(embed_dim=hidden_size, num_heads=num_heads, batch_first=True)
        # A simple MLP: Linear -> GELU -> Linear
    ...
    def forward(self, tokens):
        # tokens: shape (L, d) for a single sample.
        # Get the device from the input tokens
        device = tokens.device
        
        # Add batch dimension: shape (1, L, d)
        tokens = tokens.unsqueeze(0)
        batch_size = tokens.size(0)  # will be 1 in this per-sample loop
        
        # Ensure the entire module is on the same device as the input tokens
        if next(self.parameters()).device != device:
            self.to(device)
            
        # Expand latents to match batch size: shape (1, num_latents, d)
        # No need to explicitly move latents to device since self.to(device) handles this
        latents = self.latents.unsqueeze(0).expand(batch_size, -1, -1)
        
        # Use tokens as queries and the latent dictionary as keys and values.
        # The attention mechanism computes: O = softmax((QK^T)/√d)V
        attn_output, _ = self.multihead_attn(query=tokens, key=latents, value=latents)
        
        # Apply the MLP to the attention output.
        mlp_output = self.mlp(attn_output)
        
        # Mean pool over the sequence dimension (L) to obtain a single embedding.
        pooled = mlp_output.mean(dim=1)  # shape (1, d)
        return pooled.squeeze(0)  # shape (d,)
...
class LLM2Vec(nn.Module):
    def __init__(
        ...
        if self.pooling_mode == "latent_attention":
            self.latent_attn = LatentAttentionPooling(
                hidden_size=self.config.hidden_size, num_latents=512, num_heads=8
            )
    ...
    def get_pooling(self, features, last_hidden_states):  # All models padded from left
        ...
        if self.pooling_mode == "mean":
            ...
            for i, seq_l in enumerate(seq_lengths):
                if seq_l > 0:
                    complete_weights[i, -seq_l:] = torch.arange(seq_l, device=last_hidden_states.device) + 1
        ...
        elif self.pooling_mode == "latent_attention":
            pooled_embeddings = []
            # Process each sample individually using the latent attention layer.
            for i, length in enumerate(seq_lengths):
                # Extract only the valid token embeddings (ignoring left-padded tokens)
                tokens = last_hidden_states[i, -length:, :]
                pooled_embedding = self.latent_attn(tokens)
